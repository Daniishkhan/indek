...
5. **The merchant surface stays lightweight.** In the MVP it supports one action only — submit a new delivery request — and everything else remains status and remittance visibility.
...
- **Merchant view.** A lightweight hosted page reached via a link the operator sends over WhatsApp. No login. It lets the merchant submit a new delivery request and then see parcel status plus remittance visibility. Each link is unguessable and scoped to one merchant.
...
### Journey 9 — Merchant: place a delivery request and check status (lightweight)

The merchant taps the link the operator sent them in WhatsApp. A page opens with a compact order-request form at the top and the merchant's active and recently completed parcels underneath. The merchant enters customer name, phone, pickup address, delivery area, delivery address, COD amount, item summary, and notes. As soon as the pickup and delivery details are present, Indek shows an average shipping charge based on the merchant's standard delivery fee plus a light zone heuristic. The merchant sees the estimate before they submit, but ops still owns final assignment.

When the merchant submits the request, it immediately becomes an unassigned parcel in Indek for the operator team. The operator does not get an email or push notification in v1; instead, the operator console raises an in-app "new delivery request" alert and the parcel appears in the dispatch-ready queue for assignment.

Below the form, the same page shows all active and recent parcels with status (unassigned, assigned, in transit, delivered, failed, in return, returned). The merchant can tap a parcel to see the latest basic details and proof status if it exists. There is still no login, no deep settings, and no spreadsheet-style management — just lightweight request capture plus visibility.
...
### Merchant view (link-based, lightweight)
...
- **To the operator**, in-app, when a new delivery request lands in the unassigned queue, when a rider taps "Ready to drop", or when an anomaly threshold is crossed (no rider event for 30+ minutes, cash variance forming).
